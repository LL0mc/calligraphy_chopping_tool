...
from src.confidence_handler import classify_by_confidence, get_confidence_summary, print_review_list, export_results

test_pages = [29, 52, 186]  # 第30, 53, 187页 (0-based)

# OCR配置
OCR_ENGINE = "rapidocr"  # "cnocr" 或 "rapidocr"
EXPAND_STRATEGY = "square"  # "none", "fixed", "square"
EXPAND_PADDING = 15

for page_idx in test_pages:
    page_num = page_idx + 1
    print(f"\n{'='*60}")
    print(f"测试第 {page_num} 页 (OCR: {OCR_ENGINE}, 扩展: {EXPAND_STRATEGY})")
    print('='*60)
    ...
    try:
        ...
        # 打印识别结果
        print(f"\n  识别结果:")
        for result in ocr_results[:20]:  # 只显示前20个
            text = result['ocr_text'] or '?'
            print(f"    列{result['col_idx']+1} 行{result['row_idx']+1}: {text} ({result['ocr_score']:.2f})")
