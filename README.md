...
- **[Pipeline 检测模块详解](docs/pipeline_detail.md)** — 逐步骤解释 PDF 渲染、二轮内容裁剪、PP-OCRv5 字级检测、分列与子列拆分、遗漏字符检测（间隙 + 列尾）、连通域精炼（overlap 限制）、OCR 识别复用策略、去重后处理。
...
纯 CV（投影/连通域）对行书飞白、连笔效果差。PP-OCRv5 内置字符分割模型，检测框更宽能捕获完整笔画，定位更准，且输出置信度可用于筛选。配合 CC 精炼的 overlap 限制，框定位精度达 3.9px。
